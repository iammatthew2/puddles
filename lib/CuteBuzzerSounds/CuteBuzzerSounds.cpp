...
void CuteBuzzerSoundsClass::init(int aBuzzerPin) { initBuzzer(aBuzzerPin); }
...
void CuteBuzzerSoundsClass::_tone(float noteFrequency, long noteDuration,
                                  int silentDuration) {
  if (silentDuration == 0) {
    silentDuration = 1;
  }
...
void CuteBuzzerSoundsClass::bendTones(float initFrequency, float finalFrequency,
                                      float prop, long noteDuration,
                                      int silentDuration) {
  // Examples:
...
int joyfulSounds[] = {S_DISCONNECTION, S_BUTTON_PUSHED, S_MODE1,      S_MODE2,
                      S_MODE3,         S_HAPPY_SHORT,   S_SUPER_HAPPY};
...
void CuteBuzzerSoundsClass::play(int soundName) {
  switch (soundName) {
    ...
    case PIRATES:
      ...
      for (int i = 0; i < 203;
           i++) {  // 203 is the total number of music notes in the song
